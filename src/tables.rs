...
// Get the number of bits needed to encode the encoded data's size given the data encoding
pub fn get_count_bits_size(version: usize, mode_index: usize) -> u32 {
...
// Index by version, then by error correction leve, then by encoding mode
const CHARACTER_CAPACITIES: [[[u16; 3]; 4]; 40] = [
    ...
    [[3514, 2132, 1465], [2701, 1637, 1125], [1933, 1172,  805], [1501, 910,  625]],
...
// Get the number of characters that can be held in a given qr code given the error
// correction level and the data encoding
pub fn get_character_capacity(error_index: usize, mode_index: usize, version: usize) -> u16 {
    CHARACTER_CAPACITIES[version - 1][error_index][mode_index]
}
...
// Get the size in bits that the encoded data is required to be given the
// error correction index
pub fn get_required_bit_length(version: usize, level_index: usize) -> u32 {
    let values = ECC_DATA[version - 1][level_index];
...
pub fn get_error_codeword_count(version: usize, level_index: usize) -> usize {
    ECC_DATA[version - 1][level_index][0] as usize
}
...
const GALOIS_LOG: [u8; 256] = [
    ...
    237, 199, 147, 59, 118, 236, 197, 151, 51, 102, 204, 133, 23, 46, 92, 184, 109, 218, 169,
    79, 158, 33, 66, 132, 21, 42, 84, 168, 77, 154, 41, 82, 164, 85, 170, 73, 146, 57, 114,
    228, 213, 183, 115, 230, 209, 191, 99, 198, 145, 63, 126, 252, 229, 215, 179, 123, 246,
    241, 255, 227, 219, 171, 75, 150, 49, 98, 196, 149, 55, 110, 220, 165, 87, 174, 65, 130,
    25, 50, 100, 200, 141, 7, 14, 28, 56, 112, 224, 221, 167, 83, 166, 81, 162, 89, 178, 121,
    242, 249, 239, 195, 155, 43, 86, 172, 69, 138, 9, 18, 36, 72, 144, 61, 122, 244, 245, 247,
    243, 251, 235, 203, 139, 11, 22, 44, 88, 176, 125, 250, 233, 207, 131, 27, 54, 108, 216,
    173, 71, 142, 1];

// Numbers to exponents of alpha
const GALOIS_ANTILOG: [u8; 256] = [
    ...
    183, 123, 164, 118, 196, 23, 73, 236, 127, 12, 111, 246, 108, 161, 59, 82, 41, 157, 85, 170,
...
pub fn get_galois_log(exponent: usize) -> u8 {
    GALOIS_LOG[exponent]
}

pub fn get_galois_antilog(value: usize) -> u8 {
    GALOIS_ANTILOG[value]
}
...
pub fn get_generator_polynomial(count: usize) -> VecDeque<u8> {
    match count {
        ...
        15 => VecDeque::from([8, 183, 61, 91, 202, 37, 51, 58, 58, 237, 140, 124, 5, 99, 105]),
        16 => VecDeque::from([120, 104, 107, 109, 102, 161, 76, 3, 91, 191, 147, 169, 182, 194, 225, 120]),
        17 => VecDeque::from([43, 139, 206, 78, 43, 239, 123, 206, 214, 147, 24, 99, 150, 39, 243, 163, 136]),
        18 => VecDeque::from([215, 234, 158, 94, 184, 97, 118, 170, 79, 187, 152, 148, 252, 179, 5, 98, 96, 153]),
        20 => VecDeque::from([17, 60, 79, 50, 61, 163, 26, 187, 202, 180, 221, 225, 83, 239, 156, 164, 212, 212, 188, 190]),
        22 => VecDeque::from([210, 171, 247, 242, 93, 230, 14, 109, 221, 53, 200, 74, 8, 172, 98, 80, 219, 134, 160, 105, 165, 231]),
        24 => VecDeque::from([229, 121, 135, 48, 211, 117, 251, 126, 159, 180, 169, 152, 192, 226, 228, 218, 111, 0, 117, 232, 87, 96, 227, 21]),
        26 => VecDeque::from([173, 125, 158, 2, 103, 182, 118, 17, 145, 201, 111, 28, 165, 53, 161, 21, 245, 142, 13, 102, 48, 227, 153, 145, 218, 70]),
        28 => VecDeque::from([168, 223, 200, 104, 224, 234, 108, 180, 110, 190, 195, 147, 205, 27, 232, 201, 21, 43, 245, 87, 42, 195, 212, 119, 242, 37, 9, 123]),
        30 => VecDeque::from([41, 173, 145, 152, 216, 31, 179, 182, 50, 48, 110, 86, 239, 96, 222, 125, 42, 173, 226, 193, 224, 130, 156, 37, 251, 216, 238, 40, 192, 180]),
        _ => VecDeque::new()
    }
}