...
public:
  ...
  Message(std::vector<uint8_t> encodedData, ErrorCorrection level, int version) : _encodedData(encodedData) {
    _qrVersion = version;
    _level = level;
  }
...
private:
  ...
  std::vector<uint8_t> generateCorrectionCodes(int blockStart, int blockEnd); // generate error correction codes for a block of data
  void interleaveData();
